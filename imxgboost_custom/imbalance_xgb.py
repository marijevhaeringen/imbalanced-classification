...
from imxgboost_custom.weighted_loss import Weight_Binary_Cross_Entropy
from imxgboost_custom.focal_loss import Focal_Binary_Loss
from imxgboost_custom.weighted_focal_loss import Weighted_Focal_Binary_Loss
...
class imbalance_xgboost(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, num_round=10, max_depth=10, eta=0.3, subsample=1, verbosity=1, objective_func='binary:logitraw',
                 eval_metric='logloss',
                 booster='gbtree', special_objective=None, early_stopping_rounds=None, imbalance_alpha=None,
                 focal_gamma=None):
        """
    ...
    def fit(self, data_x, data_y):
        ...
        if data_x.shape[0] != data_y.shape[0]:
            raise ValueError('The number of instances for x and y data should be the same!')
        ...
        # # matrixilize the data and train the estimator
        ...
        if self.special_objective is None:
            # fit the classfifier
            self.boosting_model = xgb.train(self.para_dict, dtrain, self.num_round, evals=self.eval_list,
        ...
        elif self.special_objective == 'weighted focal':
            # if the gamma value is None then raise an error
            if self.focal_gamma is None:
                raise ValueError('Argument focal_gamma must have a value when the objective is \'weighted focal\'!')
            if self.imbalance_alpha is None:
                raise ValueError('Argument imbalance_alpha must have a value when the objective is \'weighted focal\'!')
            # construct the object
            weighted_focal_obj = Weighted_Focal_Binary_Loss(gamma_indct=self.focal_gamma, imbalance_alpha=self.imbalance_alpha)
            # fit the classifier
            self.boosting_model = xgb.train(self.para_dict, dtrain, self.num_round, evals=self.eval_list,
                                            obj=weighted_focal_obj.weighted_focal_binary_object, feval=evalerror,
                                            verbose_eval=False, early_stopping_rounds=self.early_stopping_rounds)
        else:
            raise ValueError(
                'The input special objective mode not recognized! Could only be \'weighted\' or \'focal\' or \'weighted focal\', but got ' + str(
